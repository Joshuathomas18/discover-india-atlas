#!/usr/bin/env python3
"""
Test Product Variants Extraction
Test the new .product-variants extraction functionality
"""
...
def test_product_variants():
    """Test the product variants extraction"""
    
    print("🎨 Testing Product Variants Extraction")
    print("=" * 60)
    
    scraper = MyntraComprehensiveScraper()
    
    try:
        # Test with products that likely have color variants
        test_urls = [
        ...
        for i, url in enumerate(test_urls):
            print(f"\n{'='*60}")
            print(f"🛍️ TEST {i+1}: Product Variants Extraction")
            print(f"{'='*60}")
            ...
            if product_data.get('variants'):
                ...
                for j, variant in enumerate(product_data['variants']):
                    ...
                    print(f"       Swatch Image: {variant.get('swatch_image', 'N/A')[:60]}...")
            else:
                print(f"\n🎨 Variants: None found")
